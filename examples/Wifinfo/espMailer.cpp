...
boolean ESPMailer::sendCMD(const char* cmd, uint16_t code) {
	...
	Debugln("Error while processing following command:");
	Debug('>');
	Debugln(cmd);
	Debugln(last_reply);
	return false;
...
bool ESPMailer::SMTPTo(char* arr) {
...
void ESPMailer::HeaderTo(const char* type, char* arr, char* nameArr) {
	char* tok = strtok(arr, _delim);
...
boolean ESPMailer::send(time_t tm) {
//boolean ESPMailer::send() {
	...
	if (!_smtp.connect(Host.c_str(), Port)) {
		if (do_debug >= 0) {
			Debugln("No connection to server!");
			Debugln(Host);
			Debugln(Port);
		}
		return false;
	...
	if (SMTPAuth) {
		switch (AuthType) {
			...
			case CRAM_MD5: {
				if (do_debug >= 0) {
					Debugln("AUTH Method currently not supported");
				}
				return false;
				/*
				//CRAM:
	...
	_smtp.printf("Message-Id: <%d%d@%s>\r\n", ESP.getChipId(),ESP.getCycleCount(), Host.c_str());
	_smtp.println("X-Mailer: ESPMailer v2.0");
	...
	if (!sendCMD("QUIT",221)) return true;  //marc false->true
	return true;  //ajout marc
}
